...
if (isset($_SESSION["user"]) && isset($_GET["id"])) {
    ...
    $mail = $watchrow["customer_email"];

    $recent = Database::search("SELECT * FROM `recent` WHERE `customer_email` = '" . $mail . "' AND `product_id` = '" . $pid . "'");

    if ($recent->num_rows == 1) {
        Database::iud("DELETE FROM `watchlist` WHERE `id` = '" . $id . "'");
    } else {
        Database::iud("INSERT INTO `recent`(`product_id`,`customer_email`) VALUES('" . $pid . "','" . $mail . "')");
        Database::iud("DELETE FROM `watchlist` WHERE `id` = '" . $id . "'");
    }
